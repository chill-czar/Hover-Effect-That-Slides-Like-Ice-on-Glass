...
import { awardImages } from "./data.js";
document.addEventListener("DOMContentLoaded", () => {
  // ✅ FIXED: Proper Lenis setup
  const lenis = new Lenis({
    smooth: true, // Enables smooth scrolling
    lerp: 0.1, // Adjusts smoothness (lower = smoother)
    autoResize: true, // ✅ FIXED: autoResize instead of autoRaf
  });
  ...
  awardElements.forEach((award, index) => {
    ...
    award.addEventListener("mouseenter", (e) => {
      ...
      if (enterFromTop || currentPosition === POSITIONS.BOTTOM) {
        currentPosition = POSITIONS.BOTTOM;
        gsap.to(wrapper, {
          y: POSITIONS.MIDDLE,
          duration: 0.4,
          ease: "power2.out",
